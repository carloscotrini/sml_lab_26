...
\begin{numermark}[title={Education, Wages, and Distance to College}]
We want the causal effect of education ($X$) on wages ($Y$, in \$K). Problem: ability ($U$) boosts both. We use distance to college ($Z$) as an instrument.

\begin{center}
\begin{tabular}{@{}ccccc@{}}
\toprule
Person & $Z$ (distance) & $U$ (ability) & $X = 2+Z+U$ & $Y = 10+3X+U$ \\
\midrule
A & 1 & 1 & 4 & 23 \\
B & 2 & $-2$ & 2 & 14 \\
C & 3 & 0 & 5 & 25 \\
...
True causal effect: $\beta = 3$. Verify: $\Cov(Z, U) = 0$ by construction.

\textbf{OLS} (biased): slope $= \Cov(X,Y)/\Var(X) = 14/4 = 3.5$. Ability inflates the estimate by $+0.5$.

\textbf{IV}: $\biv = \Cov(Z,Y)/\Cov(Z,X) = 6/2 = 3.0$. Exactly the true $\beta$!

The instrument strips out ability contamination by using only the variation in education that comes from geography, not brains.
\end{numermark}

\begin{vizspec}[title={``The Backdoor and the Detour'' --- IV Strength Simulator}]
\textbf{Try it:} Three linked panels show the first stage ($Z$ vs $X$), reduced form ($Z$ vs $Y$), and structural ($X$ vs $Y$) relationships. The structural panel overlays OLS (blue, biased), truth (gold), and IV (green) lines.
...
\begin{tabular}{@{}cccc@{}}
\toprule
Student & $R$ (score) & Scholarship? & $Y$ (outcome) \\
\midrule
1 & 43 & No & 81.5 \\
2 & 45 & No & 82.5 \\
3 & 47 & No & 83.5 \\
...
Na\"ive above-vs-below comparison: $92.0 - 83.0 = 9.0$. But this includes the baseline trend (higher-scoring students do better anyway).
...
\textbf{Try it:} A scatterplot of $Y$ vs.\ test score $R$ with a vertical cutoff line. Two local regression lines meet at the cutoff; the gap is the treatment effect. A bandwidth slider (2 to 50) controls how many points are used: narrow bandwidth shows a clean jump with wide CI, wide bandwidth smooths the jump away.
...
Your friend claims her new running coach made her faster, but she also bought expensive new running shoes at the same time. How do you figure out what the coach actually contributed? Step one: figure out how much of her speed improvement the shoes alone would predict, and subtract that out---what's left is the part of her speed the shoes \emph{can't} explain. Step two: figure out how much of the coaching hours the shoes can predict (maybe the coach recommended the shoes, so they're connected), and subtract that out too---what's left is the coaching variation that has nothing to do with shoes. Now compare those two leftovers: the unexplained speed against the unexplained coaching. \emph{That} remaining relationship is the coach's true contribution, cleaned of the shoe effect.

This is exactly what the Frisch--Waugh--Lovell theorem proves: ``controlling for'' shoes means removing shoes' influence from \emph{both} the outcome (speed) and the variable of interest (coaching), then running the regression on the residuals---the double-residualisation gives you the same coefficient you'd get in the full multiple regression.
\end{toystory}
...
We want the effect of exercise ($X_1$, hours) on mood ($Y$, score 1--20), controlling for sleep ($X_2$, hours).
...
\textbf{Step 1:} Regress $Y$ on $X_2$: slope $= 1.45$. Residuals $\tilde{Y} = (-1.2,\; -0.1,\; 0.0,\; +5.1,\; -3.8)$.

\textbf{Step 2:} Regress $X_1$ on $X_2$: slope $= 0.45$. Residuals $\tilde{X}_1 = (-0.2,\; -0.1,\; 0.0,\; +1.1,\; -0.8)$.
...
They match---not approximately, but to every decimal place. ``Controlling for sleep'' literally means: remove everything sleep can explain from \emph{both} mood and exercise, then see what's left.
\end{numermark}
